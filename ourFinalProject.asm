...
checkMaxBPT: 
	...
	#li $v0,2
    	...
    	l.s $f2, BPTMaxS
   	c.le.s $f18, $f2          # Compare $f1 with $f2 (set less than or equal flag)
  	bc1t r22  # Branch if $f1 < $f2 to printUpnormalTest label
   	b num2check
num2check:
	#li $v0,2
    	#mov.s $f12,$f1
    	...
    	l.s $f2, BPTMaxD
   	c.le.s $f20, $f2          # Compare $f1 with $f2 (set less than or equal flag)
  	bc1t r22  # Branch if $f1 < $f2 to printUpnormalTest label
  	li $t6,0
  	li $t6,1
   	b printUpnormalTest
...
checkMinBGT:
	l.s $f2, BGTMin
	c.le.s $f2, $f1          # Compare $f2 with $f1 (set less than flag)
	bc1f printUpnormalTest       # Branch if $f2 < $f1 to checkMaxHgb label
...
calculateLengthO5:
 ...
 doneO5:   
    ...
    li $v0, 15           # write_file syscall code = 15
    move $a0, $s1        # file descriptor
    la $a1, newLineV       # the string that will be written
    li $a2, 1        # length of the toWrite string (without null terminator)
    syscall
...
calculateLengthO6:
 ...
 TasfeerElRegisters:
  # Clear register $zero (always zero)
    li $zero, 0
    # Clear other registers
 #   li $at, 0         # $at register (Assembler Temp)
    li $v0, 0         # $v0 register (Function Return Value)
    li $v1, 0         # $v1 register
    li $a0, 0         # $a0 register (Function Argument)
    li $a1, 0         # $a1 register
    li $a2, 0         # $a2 register
    li $a3, 0         # $a3 register
    li $t0, 0         # $t0 register (Temporaries)
    li $t1, 0         # $t1 register
    li $t2, 0         # $t2 register
    li $t3, 0         # $t3 register
    li $t4, 0         # $t4 register
    ...
    li $t6, 0         # $t6 register
    li $t7, 0         # $t7 register
    li $s0, 0         # $s0 register (Saved Registers)
    li $s1, 0         # $s1 register
    li $s2, 0         # $s2 register
    li $s3, 0         # $s3 register
    li $s4, 0         # $s4 register
    ...
    li $t8, 0         # $t8 register
    li $t9, 0         # $t9 register
 #   li $k0, 0         # $k0 register (Kernel Reserved)
   # li $k1, 0         # $k1 register
    li $gp, 0         # $gp register (Global Pointer)
    #li $sp, 0         # $sp register (Stack Pointer)
   # li $fp, 0         # $fp register (Frame Pointer)
   #li $ra, 0         # $ra register (Return Address)   
   li $13,0
   li $14,0
   subi $12,$12,2
   jr $ra 
